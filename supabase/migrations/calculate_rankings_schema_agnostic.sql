-- Make calculate_rankings_and_winner work with both column naming schemes (total_points/score, time_taken_seconds/time_taken_ms, finished_at/completed_at)
...
BEGIN
  ...
  -- Update rank in game_sessions (support both column names)
  WITH ranked AS (
    SELECT
      id,
      ROW_NUMBER() OVER (
        ORDER BY COALESCE(total_points, score, 0) DESC,
                 COALESCE(time_taken_seconds, time_taken_ms / 1000, 999999) ASC
      ) AS rn
    FROM public.game_sessions
    WHERE round_id = p_round_id
      AND (finished_at IS NOT NULL OR completed_at IS NOT NULL)
  )
  ...
  SELECT wallet_address, COALESCE(total_points, score, 0)::BIGINT
  INTO v_winner_wallet, v_winner_score
  FROM public.game_sessions
  WHERE round_id = p_round_id
    AND (finished_at IS NOT NULL OR completed_at IS NOT NULL)
  ORDER BY COALESCE(total_points, score, 0) DESC, COALESCE(time_taken_seconds, time_taken_ms / 1000, 999999) ASC
  LIMIT 1;
  ...
  -- Upsert top 5 into round_payouts
  FOR v_row IN
    SELECT wallet_address, COALESCE(total_points, score, 0)::BIGINT AS scr
    FROM public.game_sessions
    WHERE round_id = p_round_id
      AND (finished_at IS NOT NULL OR completed_at IS NOT NULL)
    ORDER BY COALESCE(total_points, score, 0) DESC, COALESCE(time_taken_seconds, time_taken_ms / 1000, 999999) ASC
...
COMMENT ON FUNCTION public.calculate_rankings_and_winner(UUID) IS
  'Recalculates ranks, updates daily_rounds + round_winners + round_payouts. Uses total_points/score and time_taken_seconds/time_taken_ms and finished_at/completed_at so both schemas work.';
